...
# =====================================================
def download_if_not_exists(file_id: str, out_path: str):
    if not os.path.exists(out_path):
        print(f"⬇️ Downloading {out_path} from Google Drive...")
        os.makedirs(os.path.dirname(out_path), exist_ok=True)
        url = f"https://drive.google.com/uc?id={file_id}"
        gdown.download(url, out_path, quiet=False)
    else:
        print(f"✓ Found local file: {out_path}")
...
MODELS = {
    "xgboost": {
        ...
        "model_path": "backend/model/xgb_presence_model.pkl"
    },
    "randomforest": {
        "name": "Random Forest",
        "feature_path": "backend/model/feature_columns.pkl",
        "gdrive_id": "1s91CIaYbtkaJpyaWYTGJ_NfdvHcTVXXl",
        "model_path": "backend/model/rf_presence_model.pkl"
    },
    "logistic": {
        "name": "Logistic Regression",
        "feature_path": "backend/model/feature_columns.pkl",
        "gdrive_id": "1A8bXC8je_y7B4SMMeeXkO_7XSBjKDdea",
        "model_path": "backend/model/lr_presence_model.pkl"
    },
    "lgbm": {
        "name": "LightGBM",
        "feature_path": "backend/model/feature_columns.pkl",
        "gdrive_id": "1yq5mcl49NoVaqHvZg07Kz35Cyjy0OE0x",
        "model_path": "backend/model/lgbm_presence_model.pkl"
    },
    "stacking": {
        "name": "Stacking",
        "feature_path": "backend/model/feature_columns.pkl",
        "gdrive_id": "1v1fwXCqiSHvuCOhyVddtFNxo0Au-VOW6",
        "model_path": "backend/model/stacking_presence_model.pkl"
    }
}
...
# =====================================================
# STARTUP EVENT (DOWNLOAD + LOAD 1 LẦN)
# =====================================================
@app.on_event("startup")
def startup_event():
    print("🚀 App startup: loading models & climate data")

    # ===== LOAD MODELS =====
    for key, cfg in MODELS.items():
        try:
            download_if_not_exists(cfg["gdrive_id"], cfg["model_path"])
            ...
            print(f"✗ Failed loading model {key}: {e}")

    # ===== LOAD CLIMATE DATA =====
    for key, cfg in CLIMATE_SCENARIOS.items():
        try:
            download_if_not_exists(cfg["gdrive_id"], cfg["path"])
            climate_data[key] = rasterio.open(cfg["path"])
            print(f"✓ Loaded climate scenario: {key}")
        except Exception as e:
            print(f"✗ Failed loading climate {key}: {e}")
...
def extract_bioclim_from_tif(lat, lon, scenario_key):
    if scenario_key not in climate_data:
        print(f"⚠️ Scenario '{scenario_key}' not loaded")
        return None
    ...
    try:
        ...
        for i in range(1, 20):
            ...
            bio_values[f"bio{i}"] = float(val)

        return bio_values

    except Exception as e:
        print(f"❌ Error extracting climate data: {e}")
        return None
